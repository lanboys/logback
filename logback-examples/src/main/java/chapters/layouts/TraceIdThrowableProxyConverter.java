...
/**
 * Created by oopcoder at 2022/9/11 22:58 .
 */

public class TraceIdThrowableProxyConverter extends ThrowableHandlingConverter {

    public TraceIdThrowableProxyConverter() {
        System.out.println("TraceIdThrowableProxyConverter(): ");
    }
    ...
    protected void extraDataAfter(StringBuilder builder, StackTraceElementProxy step) {
        ThrowableProxyUtil.subjoinPackagingData(builder, step);
    }

    protected void extraDataBefore(StringBuilder builder, StackTraceElementProxy step) {
        appendTraceId(builder);
    }

    protected StringBuilder appendTraceId(StringBuilder builder) {
        return builder.append("[").append(MDC.get("traceId")).append("]").append(" ");
    }
    ...
    private void printStackLine(StringBuilder buf, int ignoredCount, StackTraceElementProxy element) {
        extraDataBefore(buf, element); // allow other data to be added
        buf.append(element);
        extraDataAfter(buf, element); // allow other data to be added
        if (ignoredCount > 0) {
            printIgnoredCount(buf, ignoredCount);
        }
    }
